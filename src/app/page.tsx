...
export default function Home() {
  ...
  const handleCreateList = async () => {
    if (newListTitle && activeBoard) {
      try {
        const newList = await api.createList({ title: newListTitle, board: activeBoard._id });
  ...
  const getCardIds = (listId: string) => (cards[listId] || []).map(card => card._id);
  ...
  const handleCreateCard = async (listId: string, cardData: Partial<CardType>, onSuccess: (card: CardType) => void) => {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (active.data.current?.type === 'List' && over.data.current?.type === 'List') {
      setLists((currentLists) => {
        ...
        newLists.forEach((list, idx) => {
          ...
          if (list.order !== newOrder) api.reorderList(list._id || '', newOrder);
        });
        return newLists;
      });
      return;
    ...
    if (active.data.current?.type === 'Card') {
      ...
      newCards[toListId] = newCards[toListId].map((card, idx) => ({ ...card, order: (idx + 1) * 1024 }));
      // If card moved between lists, recalculate order for source list too
      if (fromListId !== toListId) {
        newCards[fromListId] = newCards[fromListId].map((card, idx) => ({ ...card, order: (idx + 1) * 1024 }));
      }
      ...
      try {
        await Promise.all([
          ...newCards[toListId].map(card => api.updateCard(card._id, { order: card.order, list: card.list })),
          ...(fromListId !== toListId ? newCards[fromListId].map(card => api.updateCard(card._id, { order: card.order, list: card.list })) : [])
        ]);
      } catch (error: any) {
      ...
      {cardOpLoading && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg px-8 py-6 flex flex-col items-center gap-4">
            ...
            <span className="text-gray-700 dark:text-gray-200 font-medium">Updating cards...</span>
          </div>
        </div>
      )}
      {/* Error popup */}
      {cardOpError && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50">
          <div className="bg-red-600 text-white px-6 py-3 rounded-lg shadow-lg flex items-center gap-3">
            ...
            <button className="ml-4 text-white/80 hover:text-white" onClick={() => setCardOpError('')}>&times;</button>
          </div>
        </div>
      )}
      <aside className="w-72 bg-white dark:bg-gray-800 shadow-lg flex flex-col">
        <div className="p-6">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-white mb-6">Boards</h2>
          <div className="space-y-2">
            {boards.map(board => (
              <button
                key={board._id}
        ...
        {loadingBoard ? (
          ...
          <DndContext onDragEnd={handleDragEnd} collisionDetection={closestCorners}>
            <div className="flex justify-between items-center mb-8">
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{activeBoard.title}</h1>
              <button
            ...
            <SortableContext items={lists.map(l => l._id || l.id || '')} strategy={horizontalListSortingStrategy}>
              <div className="flex gap-6 mt-4">
                {[
                  ...(lists.length > 0
                    ? lists
                        .map(list => {
                          const key: string = list._id ? String(list._id) : (list.id ? String(list.id) : '');
                          return { list, key };
                        })
                    ...
                    : [
                        <div key="empty-state" className="flex flex-col items-center justify-center w-full py-16">
                          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-8 text-center">
                            <svg className="w-16 h-16 text-gray-400 dark:text-gray-500 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                            </svg>
                            <p className="text-gray-600 dark:text-gray-300 mb-4">No lists yet. Create one to get started!</p>
                            <button
                              onClick={() => setIsCreatingList(true)}
                  ...
                  isCreatingList ? (
                    <div key="create-list-form" className="bg-white dark:bg-gray-800 shadow-lg rounded-lg w-72 h-fit p-4">
                      <input
                        type="text"
                        value={newListTitle}
          ...
          <div className="flex justify-center items-center h-full">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-8 text-center shadow-lg">
              ...
              <p className="text-gray-600 dark:text-gray-300 mb-4">No boards found. Create one to get started!</p>
              <button
                onClick={handleCreateBoard}
                className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-medium transition-colors duration-200 inline-flex items-center gap-2"
              >
